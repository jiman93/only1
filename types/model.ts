...
type AccountDelegation = {
  ...
  expiresAt?: Date;                 // Optional expiration date for the delegation
};

type PermissionExpiry = {
  expiresInDays?: number;           // Number of days after which permissions will expire
  activeDays?: ('Monday' | 'Tuesday' | 'Wednesday' | 'Thursday' | 'Friday' | 'Saturday' | 'Sunday')[]; // Days on which permissions are valid
};